...
void print_encrypted(char message[] ,char row_1[] ,char row_2[] ,char row_3[] ,int message_id[] ,int row_number[],int position){
    int i;
    for(i = 0; i < strlen(message); i++){
        ...
        if(row_number[i] == 3){
            if(message_id[i]==0)
                message[i] = 'm';
            else  message[i] = row_3[message_id[i]];
            message_id[i]++;
            continue;
        }
    }
...
int main()
{
    ...
    print_encrypted(message,row_1,row_2,row_3,message_id,row_number,position);
